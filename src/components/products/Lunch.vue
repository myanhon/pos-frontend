  ...
  <v-container fluid>
    <v-row>
      <v-col
        ...
        md="4"
        lg="2"
        v-for="(lunch, i) in this.getLunch"
        :key="i"
        ...
        <v-hover v-slot:default="{ hover }">
          <v-card :elevation="hover ? 16 : 2" flat>
            <v-img
              ...
              :src="serverUrl + lunch.productImage"
              aspect-ratio="1.7"
              v-on:click="addProductToCart(lunch._id)"
            >
            </v-img>
            <p>{{ lunch.name }} - {{ lunch.size }} ${{ lunch.price }}</p>
          </v-card>
        </v-hover>
      </v-col>
    </v-row>
